...
// Gets the node at the exact tile position (tilemap-based)
HighLevelNode@ getNodeFromPosition(Vec2f&in position, HighLevelNode@[]@ nodeMap, CMap@ map)
{
	for (uint i = 0; i < nodeMap.length; ++i)
	{
		...
		if (node.original_position == position)
			return node;
	}
	return null;
}

// Gets the closest applicable node to the position (tilemap-based)
HighLevelNode@ getClosestNode(Vec2f&in position, HighLevelNode@[]@ nodeMap, const u8&in flags = Path::GROUND)
{
	...
	{
		HighLevelNode@ node = nodeMap[i];
		if ((node.flags & flags) == 0) continue;

		const f32 distance = (node.position - position).LengthSquared();
