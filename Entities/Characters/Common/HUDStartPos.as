...
void DrawInventoryOnHUD( CBlob@ this, Vec2f tl, Vec2f hudPos = Vec2f(0,0) )
{
	...
	for (int i = 0; i < inv.getItemsCount(); i++)
	{
		...
		if (quantity != 1)
		{
			if (quantity < 10) { GUI::DrawText(""+quantity, itempos +Vec2f(22,18), col ); }
			else if (quantity < 100) { GUI::DrawText(""+quantity, itempos +Vec2f(14,18), col ); }
			else { GUI::DrawText(""+quantity, itempos +Vec2f(6,18), col ); }
		}
	}
}
...
void DrawCoinsOnHUD( CBlob@ this, const int coins, Vec2f tl, const int slot )
{
	if (coins > 0)
	{
		...
		GUI::DrawText(""+coins, Vec2f(72,44), SColor(255,255,165,60) );
	}
}