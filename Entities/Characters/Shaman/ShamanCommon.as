...
namespace ShamanParams
{
	...
	const ::Spell[] spells = 
	{
		Spell("flameorb", "Flame Orb", 75, "Ignites enemies on impact.",
			SpellCategory::offensive, SpellType::other, 6, 20, 0, 256.0f, true, false, array<int> = {SpellAttribute::SPELL_PROJECTILE}),
		...
		Spell("haste", "Haste", 20, "Give your allies some added speed and maneuverability. Fully charge to hasten yourself.",
			SpellCategory::support, SpellType::other, 10, 20, 0, 360.0f, true, false, array<int> = {SpellAttribute::SPELL_MOVEMENT, SpellAttribute::SPELL_CASTEREFFECT, SpellAttribute::SPELL_ALLYEFFECT}),

		Spell("waterbolt", "Water Bolt", 126, "A dense sphere of water. Speeds up and costs 1 mana less if water is covering the caster.",
			SpellCategory::offensive, SpellType::other, 5, 30, 0, 256.0f, false, false, array<int> = {SpellAttribute::SPELL_PROJECTILE, SpellAttribute::SPELL_WATER}),

		Spell("fire_ward", "Fire Ward", 57, "Form a heat protection aura around yourself. Completely nullifies fire damage.",
			SpellCategory::support, SpellType::other, 15, 30, 0, 360.0f, true, false, array<int> = {SpellAttribute::SPELL_CASTEREFFECT, SpellAttribute::SPELL_ALLYEFFECT}),
		...
		Spell("chainlightning", "Chain Lightning", 84, "Send a lightning at the enemy which may hit a close target after a successful hit. Overcharge increases damage and the max amount of targets.",
			SpellCategory::offensive, SpellType::other, 25, 25, 0, 164.0f, false, false, array<int> = {SpellAttribute::SPELL_PROJECTILE, SpellAttribute::SPELL_ELECTRICITY}),
		...
		Spell("firetotem", "Totem of Fire", 76, "Shoots flames at nearby enemies. Ignites everything in close range upon death. Overcharge increases lifetime and fire rate.",
			SpellCategory::summoning, SpellType::other, 30, 50, 2, 156.0f, true, true, array<int> = {SpellAttribute::SPELL_SENTRY, SpellAttribute::SPELL_GROUNDED, SpellAttribute::SPELL_FIRE}),
		...
		Spell("watertotem", "Totem of Water", 77, "Heals most damaged nearby ally. Only one totem can heal a target. Generates heal charges passively. Overchage increases durability and generation rate. Pushes everything away upon death.",
			SpellCategory::summoning, SpellType::other, 40, 90, 8, 128.0f, true, true, array<int> = {SpellAttribute::SPELL_SENTRY, SpellAttribute::SPELL_GROUNDED, SpellAttribute::SPELL_HEAL, SpellAttribute::SPELL_WATER, SpellAttribute::SPELL_KNOCKBACK}),
		...
		Spell("arclightning", "Arc Lightning", 81, "Send a lightning arc that links to other arcs nearby.",
			SpellCategory::offensive, SpellType::other, 18, 28, 0, 256.0f, true, false, array<int> = {SpellAttribute::SPELL_PROJECTILE, SpellAttribute::SPELL_ELECTRICITY}),
		...
		Spell("balllightning", "Lightning Ball", 123, "Creates an electrified and moving ball of pure lightning.",
			SpellCategory::offensive, SpellType::other, 35, 40, 4, 128.0f, true, false, array<int> = {SpellAttribute::SPELL_PROJECTILE, SpellAttribute::SPELL_ELECTRICITY}),
