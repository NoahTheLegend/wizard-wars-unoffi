...
{
	RunnerTextures@ runner_tex = addRunnerTextures(this, "shaman", "Shaman");
...
const bool firing = (blob.isKeyPressed(key_action1) || blob.isKeyPressed(key_action2)) && !getRules().get_bool("showHelp");;
	...
	if (blob.isKeyPressed(key_action1))
		spell_ready = ShamanParams::spells[playerPrefsInfo.primarySpellID].needs_full ? shaman.charge_state >= ShamanParams::cast_3 : shaman.charge_state >= ShamanParams::cast_1;
	else if (blob.isKeyPressed(key_action2))
		spell_ready = ShamanParams::spells[Maths::Min(playerPrefsInfo.hotbarAssignments_Shaman[15], spellsLength-1)].needs_full ? shaman.charge_state >= ShamanParams::cast_3 : shaman.charge_state >= ShamanParams::cast_1;
	bool full_charge = shaman.charge_state == ShamanParams::extra_ready;
	needs_shiny = spell_ready;
	needs_shiny2 = shaman.charge_state >= ShamanParams::charging;
