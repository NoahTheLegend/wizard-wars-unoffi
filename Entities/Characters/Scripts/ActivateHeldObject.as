...
bool ActivateBlobCustom(CBlob@ this, CBlob@ blob, Vec2f pos, Vec2f vector, Vec2f vel)
{
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("activate/throw"))
	{
		...
		if (carried !is null)
		{
			ActivateBlobCustom(this, carried, pos, vector, vel);
		}
		else // search in inv
		{
			...
			for (int i = 0; i < inv.getItemsCount(); i++)
			{
				...
				if (ActivateBlobCustom(this, blob, pos, vector, vel))
					return;
			}
		}
	}
...
void DoThrow(CBlob@ this, CBlob@ carried, Vec2f pos, Vec2f vector, Vec2f selfVelocity)
{
	...
	if (carried !is null)
	{
		...
		else if (carried.hasTag("super heavy weight"))
		{
			vel *= 0.3f;
		}
