...
void DrawSpellBar(CBlob@ this)
{
	...
	u8 primarySpell;
	if (  playerPrefsInfo.hotbarAssignments_Wizard.length >= playerPrefsInfo.primaryHotkeyID )
		primarySpell = playerPrefsInfo.hotbarAssignments_Wizard[playerPrefsInfo.primaryHotkeyID];
	else
		primarySpell = 0;
	...
	for (uint i = 0; i < 15; i++)	//only 15 total spells held inside primary hotbar
	{
		...
		if ( i < 5 )		//spells 0 through 4
		{
			...
			if ( WizardMana < spellMana )
				GUI::DrawRectangle(primaryPos + Vec2f(0,64) + Vec2f(32,0)*i, primaryPos + Vec2f(32,64) + Vec2f(0, 32*(1-(WizardMana/spellMana))) + Vec2f(32,0)*i, SColor(200, 0, 0, 0));	
		...
		else if ( i < 10 )	//spells 5 through 9	
		{
			...
			if ( WizardMana < spellMana )
				GUI::DrawRectangle(primaryPos + Vec2f(0,32) + Vec2f(32,0)*(i-5), primaryPos + Vec2f(32,32) + Vec2f(0, 32*(1-(WizardMana/spellMana))) + Vec2f(32,0)*(i-5), SColor(200, 0, 0, 0));
		...
		else				//spells 10 through 14
		{
			...
			if ( WizardMana < spellMana )
				GUI::DrawRectangle(primaryPos + Vec2f(32,0)*(i-10), primaryPos + Vec2f(32,0) + Vec2f(0, 32*(1-(WizardMana/spellMana))) + Vec2f(32,0)*(i-10), SColor(200, 0, 0, 0));
	...
	if ( WizardMana < secondarySpellMana )
		GUI::DrawRectangle(secondaryPos, secondaryPos + Vec2f(32, 32*(1-(WizardMana/secondarySpellMana))), SColor(200, 0, 0, 0));
	...
	if ( WizardMana < aux1SpellMana )
		GUI::DrawRectangle(aux1Pos, aux1Pos + Vec2f(32, 32*(1-(WizardMana/aux1SpellMana))), SColor(200, 0, 0, 0));
	...
	if ( WizardMana < aux2SpellMana )
		GUI::DrawRectangle(aux2Pos, aux2Pos + Vec2f(32, 32*(1-(WizardMana/aux2SpellMana))), SColor(200, 0, 0, 0));
