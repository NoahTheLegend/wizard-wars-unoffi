...
{
	PaladinInfo paladin;
	this.set("paladinInfo", @paladin);
	
	ManaInfo manaInfo;
	manaInfo.maxMana = PaladinParams::MAX_MANA;
	manaInfo.manaRegen = PaladinParams::MANA_REGEN;
	this.set("manaInfo", @manaInfo);

	this.set_bool("burnState", false);
	this.set_s8( "charge_time", 0 );
	this.set_u8( "charge_state", PaladinParams::not_aiming );
	this.set_s32( "mana", 150 );
...
{
	if (player !is null){
		player.SetScoreboardVars("ScoreboardIcons.png", 13, Vec2f(16,16));
	}
}

void ManageSpell( CBlob@ this, PaladinInfo@ paladin, PlayerPrefsInfo@ playerPrefsInfo, RunnerMoveVars@ moveVars )
{
	CSprite@ sprite = this.getSprite();
	bool ismyplayer = this.isMyPlayer();
	s32 charge_time = paladin.charge_time;
	u8 charge_state = paladin.charge_state;
    ...
    {
        spellID = playerPrefsInfo.hotbarAssignments_Paladin[Maths::Min(15,hotbarLength-1)];
    ...
    {
        spellID = playerPrefsInfo.hotbarAssignments_Paladin[Maths::Min(16,hotbarLength-1)];
    ...
    {
        spellID = playerPrefsInfo.hotbarAssignments_Paladin[Maths::Min(17,hotbarLength-1)];
	...
	Spell spell = PaladinParams::spells[spellID];
	...
	//cancel charging
	if ( controls.isKeyPressed( KEY_MBUTTON ) || paladin.spells_cancelling == true )
	{
		charge_time = 0;
		charge_state = PaladinParams::not_aiming;
		
		if (paladin.spells_cancelling == false)
		{
			sprite.PlaySound("PopIn.ogg", 1.0f, 1.0f);
		}
		paladin.spells_cancelling = true;	
		
		// only stop cancelling once all spells buttons are released
		if ( !is_pressed )
		{
			paladin.spells_cancelling = false;
		}
	}

	bool onCooldown = playerPrefsInfo.spell_cooldowns[spellID] <= 0;
	bool canCastSpell = paladinMana >= spell.mana && onCooldown;
    ...
    if (is_pressed && canCastSpell) 
    {
        ...
        {
            charge_state = PaladinParams::extra_ready;
            charge_time = spell.full_cast_period;
        }
        else if (charge_time >= spell.cast_period)
        {
            charge_state = PaladinParams::cast_3;
        }
        else if (charge_time >= spell.cast_period_2)
        {
            charge_state = PaladinParams::cast_2;
        }
        else if (charge_time >= spell.cast_period_1)
        {
            charge_state = PaladinParams::cast_1;
        }
    }
    else if (just_released)
    {
        if (canCastSpell && charge_state > PaladinParams::charging && not (spell.needs_full && charge_state < PaladinParams::cast_3) &&
            (this.isMyPlayer() || this.getPlayer() is null || this.getPlayer().isBot()))
        {
			...
			u8 castSpellID;
			if ( is_aux2 )
				castSpellID = playerPrefsInfo.hotbarAssignments_Paladin[Maths::Min(17,hotbarLength-1)];
			else if ( is_aux1 )
				castSpellID = playerPrefsInfo.hotbarAssignments_Paladin[Maths::Min(16,hotbarLength-1)];
			else if ( is_secondary )
				castSpellID = playerPrefsInfo.hotbarAssignments_Paladin[Maths::Min(15,hotbarLength-1)];
			else
			...
			int spell_cd_time = PaladinParams::spells[castSpellID].cooldownTime * getTicksASecond();
    ...
    paladin.charge_time = charge_time;
    paladin.charge_state = charge_state;
    ...
    if ( ismyplayer )
    {
		if (!getHUD().hasButtons()) 
		{
            ...
            else if (paladin.charge_time > spell.cast_period)
            {
                frame = 12 + paladin.charge_time % 15 / 5;
            }
			else if (paladin.charge_time > 0) {
				frame = paladin.charge_time * 12 /spell.cast_period; 
			}
			getHUD().SetCursorFrame( frame );
			/*u8 pulses = paladin.pulse_amount;
	...
	if ( !is_pressed && getRules().get_bool("spell_number_selection") )
	{
		...
		PaladinInfo@ paladin;
		if (!this.get( "paladinInfo", @paladin )) 
		{
			return;
		}
		...
		int nextHotkey =  playerPrefsInfo.hotbarAssignments_Paladin.length;
		...
		if ( nextHotkey <  playerPrefsInfo.hotbarAssignments_Paladin.length )
		{
			playerPrefsInfo.primaryHotkeyID = nextHotkey;
			playerPrefsInfo.primarySpellID = playerPrefsInfo.hotbarAssignments_Paladin[nextHotkey];
...
void onTick( CBlob@ this )
{
	...
	{
		s8 pulses = paladin.pulse_amount;
        
		if (pulses < 3)
		{
			paladin.pulse_amount += 1;
        }
    }*/

	/*if(getKnockedRemaining(this) > 0)
	{
		paladin.charge_state = 0;
		paladin.charge_time = 0;
		return;
	}*/
    ...
    ManageSpell(this, paladin, playerPrefsInfo, moveVars);
}
