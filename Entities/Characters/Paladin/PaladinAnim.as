...
{
	RunnerTextures@ runner_tex = addRunnerTextures(this, "paladin", "Paladin");
...
{
    string texname = "PaladinMale.png";
	...
	CSpriteLayer@ weapon = this.addSpriteLayer("weapon", "PaladinMale.png", 32, 32);

	if (weapon !is null)
	{
        Animation@ anim = weapon.addAnimation("default", 0, false);
		for (u8 i = 0; i < 50; i++) // 50 is whole spritesheet
		{
			anim.AddFrame(i);
		}
		weapon.SetVisible(true);
        weapon.SetRelativeZ(5.1f);
		weapon.SetOffset(Vec2f(-3,-2));
	}
}
...
void onTick( CSprite@ this )
{
	...
	if (blob.isKeyPressed(key_action1))
		spell_ready = PaladinParams::spells[playerPrefsInfo.primarySpellID].needs_full ? entro.charge_state >= PaladinParams::cast_3 : entro.charge_state >= PaladinParams::cast_1;
	else if (blob.isKeyPressed(key_action2))
		spell_ready = PaladinParams::spells[Maths::Min(playerPrefsInfo.hotbarAssignments_Paladin[15], spellsLength-1)].needs_full ? entro.charge_state >= PaladinParams::cast_3 : entro.charge_state >= PaladinParams::cast_1;
	bool full_charge = entro.charge_state == PaladinParams::extra_ready;
	needs_shiny = spell_ready;
	needs_shiny2 = entro.charge_state >= PaladinParams::charging;
	...
	{
		u8 spriteframe = this.getFrame();
		CSpriteLayer@ weapon = this.getSpriteLayer("weapon");
		if (weapon !is null)
		{
			if (blob.hasTag("dead"))
			{
			...
			{
				weapon.animation.frame = spriteframe+25;
