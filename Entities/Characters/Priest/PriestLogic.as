...
void onInit( CBlob@ this )
{
	...
	ManaInfo manaInfo;
	manaInfo.maxMana = PriestParams::MAX_MANA;
	manaInfo.manaRegen = PriestParams::MANA_REGEN;
	this.set("manaInfo", @manaInfo);

	this.set_bool("burnState", false);
	this.set_s8( "charge_time", 0 );
	this.set_u8( "charge_state", PriestParams::not_aiming );
...
void ManageSpell( CBlob@ this, PriestInfo@ priest, PlayerPrefsInfo@ playerPrefsInfo, RunnerMoveVars@ moveVars )
{
	CSprite@ sprite = this.getSprite();
	bool ismyplayer = this.isMyPlayer();
	s32 charge_time = priest.charge_time;
	u8 charge_state = priest.charge_state;
    ...
    {
        spellID = playerPrefsInfo.hotbarAssignments_Priest[Maths::Min(15,hotbarLength-1)];
    ...
    {
        spellID = playerPrefsInfo.hotbarAssignments_Priest[Maths::Min(16,hotbarLength-1)];
    ...
    {
        spellID = playerPrefsInfo.hotbarAssignments_Priest[Maths::Min(17,hotbarLength-1)];
	...
	//cancel charging
	if ( controls.isKeyPressed( KEY_MBUTTON ) || priest.spells_cancelling == true )
	{
		charge_time = 0;
		charge_state = PriestParams::not_aiming;
		
		if (priest.spells_cancelling == false)
		{
			sprite.PlaySound("PopIn.ogg", 1.0f, 1.0f);
		}
		priest.spells_cancelling = true;	
		
		// only stop cancelling once all spells buttons are released
		if ( !is_pressed )
		{
			priest.spells_cancelling = false;
		}
	}

	bool onCooldown = playerPrefsInfo.spell_cooldowns[spellID] <= 0;
	bool canCastSpell = priestMana >= spell.mana && onCooldown;
    ...
    if (is_pressed && canCastSpell) 
    {
        ...
        {
            charge_state = PriestParams::extra_ready;
            charge_time = spell.full_cast_period;
        }
        else if (charge_time >= spell.cast_period)
        {
            charge_state = PriestParams::cast_3;
        }
        else if (charge_time >= spell.cast_period_2)
        {
            charge_state = PriestParams::cast_2;
        }
        else if (charge_time >= spell.cast_period_1)
        {
            charge_state = PriestParams::cast_1;
        }
    }
    else if (just_released)
    {
        if (canCastSpell && charge_state > PriestParams::charging && not (spell.needs_full && charge_state < PriestParams::cast_3) &&
            (this.isMyPlayer() || this.getPlayer() is null || this.getPlayer().isBot()))
        {
			...
			u8 castSpellID;
			if ( is_aux2 )
				castSpellID = playerPrefsInfo.hotbarAssignments_Priest[Maths::Min(17,hotbarLength-1)];
			else if ( is_aux1 )
				castSpellID = playerPrefsInfo.hotbarAssignments_Priest[Maths::Min(16,hotbarLength-1)];
    ...
    priest.charge_time = charge_time;
    priest.charge_state = charge_state;
    ...
    if ( ismyplayer )
    {
		if (!getHUD().hasButtons()) 
		{
            ...
            else if (priest.charge_time > spell.cast_period)
            {
                frame = 12 + priest.charge_time % 15 / 5;
            }
			else if (priest.charge_time > 0) {
				frame = priest.charge_time * 12 /spell.cast_period; 
			}
			getHUD().SetCursorFrame( frame );
			/*u8 pulses = priest.pulse_amount;
	...
	if ( !is_pressed && getRules().get_bool("spell_number_selection") )
	{
		...
		if ( nextHotkey <  playerPrefsInfo.hotbarAssignments_Priest.length )
		{
			playerPrefsInfo.primaryHotkeyID = nextHotkey;
			playerPrefsInfo.primarySpellID = playerPrefsInfo.hotbarAssignments_Priest[nextHotkey];
...
void onTick( CBlob@ this )
{
	...
	{
		s8 pulses = priest.pulse_amount;
        
		if (pulses < 3)
		{
			priest.pulse_amount += 1;
        }
    }*/

	/*if(getKnockedRemaining(this) > 0)
	{
		priest.charge_state = 0;
		priest.charge_time = 0;
		return;
	}*/
    ...
    ManageSpell( this, priest, playerPrefsInfo, moveVars );
}
