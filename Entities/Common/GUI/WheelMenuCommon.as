...
class WheelMenuEntry
{
	...
	void update(float angle, float step, float item_distance)
	{
		angle_min = angle;
...
class WheelMenu
{
    ...
    bool for_spells = false;

    float item_distance = WheelMenu::default_item_distance;

	float hover_distance =  WheelMenu::default_hover_distance;
	...
	void determine_hovered()
	{
		...
		if (is_cursor_in_range(cursor, hover_distance))
		{
	...
	void update()
	{
		...
		{
			entries[i].update(angle, step, item_distance);
			angle += step;
		}
	...
	void draw_center_pane()
	{
        ...
        if(for_spells == true)
        {
            ...
            GUI::DrawTextCentered(hover_text, origin + Vec2f(WheelMenu::center_pane_text_margin.x, WheelMenu::center_pane_text_margin.y * 1.5f), WheelMenu::pane_text_color);
        }
        else
        {
