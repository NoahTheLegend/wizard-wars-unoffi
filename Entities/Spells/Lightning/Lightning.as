	...
	if ( hasHit )
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			for (u8 i = 0; i < div_shields.length; i++)
			{
				...
				if (b.getTeamNum() == this.getTeamNum() || (hi.hitpos-b.getPosition()).Length() > 156.0f) continue;
				{
					if (b.getPosition().x <= hi.hitpos.x) // go right
					{
						this.setPosition(this.getPosition()+Vec2f(12.0f,0));
					}
					else
					{
						this.setPosition(this.getPosition()+Vec2f(-4.0f,0));
					}
					break;
				}
			}
			...
			if (hi.blob !is null) // blob
			{
				...
				{
					f32 damage = DAMAGE;
					if (hitBlob.get_bool("waterbarrier") || hitBlob.isInWater()) damage *= 1.5f;
...
void lightningSparks(Vec2f pos, int amount)
{
	...
	for (int i = 0; i < amount; i++)
    {
        ...
        CParticle@ p = ParticlePixelUnlimited( pos, vel, SColor( 255, 200+_sprk_r.NextRanged(55), 200+_sprk_r.NextRanged(55), 255), true );
